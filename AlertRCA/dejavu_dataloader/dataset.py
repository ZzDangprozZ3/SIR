...
def read_faults(datadir):
    # df = pd.read_csv(os.path.join(datadir, 'faults.csv'))  #original
    df = pd.read_csv(
        os.path.join(datadir, 'faults.csv'),
        dtype={"root_cause_node": str}  # ép kiểu string
    )
    return df
...
class graph_dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        window_size: Tuple[int, int] = (10, 10), #original est (10,10)
        granularity: int = 60, # original est 60
    ):
...
def get_dejavu_dataset(datadir, ratio=0.5, graph_list=False, upsample=15):
    ...
    if not graph_list:
        ...
        #     nxg_list.append(nxg.copy())
        #     if i % 10 == 0:
        #         print(f"Copied {i+1}/{len(fdf)} graphs")
        # nxg_list = [nxg.copy() for i in range(len(fdf))]
        nxg_list = [nxg] * len(fdf) 
    else:
        nxg_list = []
        for i in range(len(fdf)):
